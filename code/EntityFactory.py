        ...
        match entity_name:
            case "Level1Bg":
                ...
                for i in range(5):
                    ...
                    print(f"Carregando: Level1Bg{i} (posição 0,0)")#Verificar erros no fundo
                    bg2 = Background(f'Level1Bg{i}', (WIN_WIDTH, 0))
                    print(f"Carregando cópia: Level1Bg{i} (posição {WIN_WIDTH},0)")#Verificar erros no fundo
                    # Adiciona cada fundo e sua cópia, mantendo a ordem correta
                    list_bg.extend([bg1, bg2])
                return list_bg
            case 'Player1':
                return Player('Player1', (10, WIN_HEIGHT - 100))
            case 'Enemy1':
